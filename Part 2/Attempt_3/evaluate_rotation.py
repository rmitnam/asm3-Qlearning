"""
Evaluation script for Arena RL Environment with Rotation Controls
...
def find_best_model(model_dir: str) -> str:
    ...
    priorities = [
        "best_model.zip",
        "rotation_ppo_final.zip",
        "rotation_dqn_final.zip",
    ]
...
def evaluate_agent(
    ...
    """
    print(f"=" * 60)
    print(f"Evaluating Agent - Rotation Controls")
    print(f"=" * 60)
    ...
    for episode in range(n_episodes):
        ...
        while not done:
            if human_play:
                action = get_human_action_rotation()
            else:
...
def list_available_models(base_dir: str = MODEL_DIR):
    ...
    for item in os.listdir(base_dir):
        ...
        if os.path.isdir(item_path):
            # Check for rotation models
            if 'rotation' in item.lower():
    ...
    if not found:
        print("  No rotation models found.")
        print("  Run train_rotation.py first to train a model.")


def main():
    parser = argparse.ArgumentParser(description="Evaluate trained RL agent (Rotation Controls)")
    parser.add_argument(
        "--model", "-m",
        type=str,
        default=None,
    ...
    if model_path is None:
        ...
        default_paths = [
            os.path.join(MODEL_DIR, "rotation_ppo"),
            os.path.join(MODEL_DIR, "rotation_dqn"),
        ]
