...
Gym-style environment for RL training with direct 4-way movement.
...
class ArenaEnvDirectional(gym.Env):
    """
    Arena environment with directional controls.
    
    The player controls a ship that can move in 4 directions and shoot.
    ...
    def __init__(self, render_mode: Optional[str] = None):
        ...
        # Structure (no orientation needed for directional movement):
        # [0-1]: Player position (x, y) normalized
        # [2-3]: Player velocity (vx, vy) normalized
        # [4]: Player health normalized
        # [5]: Current phase normalized
        # [6-9]: Nearest enemy (distance, angle_sin, rel_x, rel_y)
        # [10-13]: Second nearest enemy
        # [14-17]: Nearest spawner (distance, angle_sin, rel_x, rel_y)
        # [18-21]: Second nearest spawner
        # [22]: Number of enemies normalized
        # [23]: Number of spawners normalized
        # [24]: Can shoot (1 if cooldown = 0)
        # [25]: Time remaining normalized
        self.observation_space = spaces.Box(
            low=-1.0, high=1.0,
            shape=(OBSERVATION_SIZE_DIRECTIONAL,),
            dtype=np.float32
        )
        ...
        # For directional shooting - aim toward nearest enemy or last movement direction
        self.last_aim_angle = -90  # Default aim up
    ...
    def step(self, action: int) -> Tuple[np.ndarray, float, bool, bool, Dict]:
        ...
        self._update_aim_direction(action)
        
        # Update player and get projectile if shooting
        projectile = self.player.update_directional(action)
        if projectile:
            # Override projectile direction to aim at nearest enemy/spawner
            projectile.angle = self.last_aim_angle
            rad = math.radians(self.last_aim_angle)
            projectile.vx = math.cos(rad) * projectile.speed
            projectile.vy = math.sin(rad) * projectile.speed
    ...
    def _update_aim_direction(self, action: int):
        """Update aim direction based on movement or nearest threat."""
        ...
        if action == 1:  # Up
            ...
            self.last_aim_angle = 180
        elif action == 4:  # Right
            self.last_aim_angle = 0
        elif action == 5:  # Shooting - aim at nearest threat
            # Find nearest enemy or spawner
            nearest_target = None
            min_dist = float('inf')
            ...
            # If no enemy nearby, aim at nearest spawner
            if nearest_target is None or min_dist > 300:
                for spawner in self.spawners:
                    if spawner.alive:
                        ...
                        if dist < min_dist:
                            min_dist = dist
                            nearest_target = spawner
                            
            if nearest_target is not None:
                dx = nearest_target.x - self.player.x
                dy = nearest_target.y - self.player.y
                self.last_aim_angle = math.degrees(math.atan2(dy, dx))
    ...
    def _get_observation(self) -> np.ndarray:
        ...
        obs = np.zeros(OBSERVATION_SIZE_DIRECTIONAL, dtype=np.float32)
        ...
        obs[4] = self.player.health / PLAYER_MAX_HEALTH
        
        # Current phase normalized
        obs[5] = self.current_phase / MAX_PHASE
        ...
        obs[22] = min(len(self.enemies) / 20, 1.0)
        obs[23] = min(len(alive_spawners) / 10, 1.0)
        
        # Can shoot
        obs[24] = 1.0 if self.player.shoot_cooldown <= 0 else 0.0
        
        # Time remaining normalized
        obs[25] = 1.0 - (self.steps / MAX_EPISODE_STEPS)
    ...
    def _get_nearest_entities(self, entities: List, count: int) -> List[Tuple[float, float, float, float]]:
        ...
        for i in range(count):
            if i < len(distances):
                ...
                # Angle as sin/cos for better neural network learning
                angle = math.atan2(dy, dx)
                norm_angle = math.sin(angle)
    ...
    def render(self):
        ...
        if self.window is None:
            ...
            pygame.display.set_caption("Arena RL - Directional Controls")
    ...
    def _draw_aim_indicator(self):
        """Draw a small line showing current aim direction."""
        if self.player and self.player.alive:
            rad = math.radians(self.last_aim_angle)
            end_x = self.player.x + math.cos(rad) * 40
            end_y = self.player.y + math.sin(rad) * 40
    ...
    def _draw_hud(self):
        ...
        # Controls hint
        controls_text = self.font.render("Directional Mode: Up/Down/Left/Right/Shoot", True, LIGHT_GRAY)
        self.window.blit(controls_text, (WINDOW_WIDTH // 2 - 160, WINDOW_HEIGHT - 25))
