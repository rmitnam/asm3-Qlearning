"""
Training script for Arena RL Environment with Rotation Controls
...
from arena_env_rotation import ArenaEnvRotation
from config import *
...
def make_env(rank: int, seed: int = 0):
    ...
    """
    def _init():
        env = ArenaEnvRotation(render_mode=None)  # No rendering during training
...
def train_ppo(
    ...
    """
    Train a PPO agent on the rotation-controlled arena environment.
    ...
    print(f"=" * 60)
    print(f"Training PPO Agent - Rotation Controls")
    print(f"=" * 60)
    ...
    checkpoint_callback = CheckpointCallback(
        ...
        name_prefix="rotation_ppo_checkpoint"
    )
    ...
    # Save final model
    final_model_path = os.path.join(model_path, "rotation_ppo_final")
...
def train_dqn(
    ...
    """
    Train a DQN agent on the rotation-controlled arena environment.
    ...
    print(f"=" * 60)
    print(f"Training DQN Agent - Rotation Controls")
    print(f"=" * 60)
    print(f"Total Timesteps: {total_timesteps:,}")
    print(f"Learning Rate: {learning_rate}")
    print(f"Seed: {seed}")
    ...
    env = Monitor(ArenaEnvRotation(render_mode=None))
    
    # Create evaluation environment
    print("Creating evaluation environment...")
    eval_env = Monitor(ArenaEnvRotation(render_mode=None))
    ...
    checkpoint_callback = CheckpointCallback(
        save_freq=50000,
        save_path=model_path,
        name_prefix="rotation_dqn_checkpoint"
    )
    ...
    # Save final model
    final_model_path = os.path.join(model_path, "rotation_dqn_final")
    model.save(final_model_path)
    print(f"\nFinal model saved to: {final_model_path}")
...
def main():
    parser = argparse.ArgumentParser(description="Train RL agent for Arena (Rotation Controls)")
    parser.add_argument(
        "--algorithm", "-a",
        type=str,
        default="ppo",
